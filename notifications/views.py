...
def get_all_templates(request):
    templates = NotificationTemplate.objects.all().values('id', 'name', 'notification_type_id')
...
def send_notifications(request):
    ...
    try:
        ...
        message = template.content  # assuming your NotificationTemplate has a 'content' field

        # ✅ Send SMS via Fast2SMS
        if channel.lower() == 'sms':
            sms_response = send_sms(recipients, message)
            if sms_response.get('return') is not True:
                return JsonResponse({
                    'success': False,
